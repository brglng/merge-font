...
- **每個子家族獨立設定** — 每個子家族（如 Regular、Italic、Bold）可以使用不同的 CJK 字型和獨立的縮放/偏移參數，支援混合搭配設計，例如正體使用宋體 CJK 字型、斜體使用手寫風格 CJK 字型。
